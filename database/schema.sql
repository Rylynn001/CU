CREATE DATABASE IF NOT EXISTS comfyui CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;
USE comfyui;
...
create index assets_workspace_filter_index
	on assets (rfid, asset_type, tag, created_at, id);

create index assets_recent_index
	on assets (rfid, created_at, id);

create table if not exists category_assets
(
	category_id bigint not null,
	assets_id bigint not null,
	primary key (category_id, assets_id)
)
comment '资产对应分类';

create index assets_category_assets_id_index
	on category_assets (assets_id);
...
create table if not exists history_assets_rel
(
	history_id bigint not null,
	asset_id bigint not null,
	primary key (history_id, asset_id),
	constraint history_assets_rel_pk_2
		unique (asset_id)
);
...
create index history_input_assets_rel_asset_id_index
	on history_input_assets_rel (asset_id);
...
comment '制作板自动保存与协作状态';

create table if not exists production_board_snapshots
(
