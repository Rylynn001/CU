...
async def validate_inputs(prompt_id, prompt, item, validated):
    ...
    参数：
        prompt_id:  本次任务的唯一 ID
        prompt:     完整的工作流字典
        item:       当前要验证的节点编号
        validated:  已验证节点的结果缓存字典，避免重复验证

    返回：三元组 (is_valid, errors, unique_id)
        is_valid:   bool，该节点是否通过验证
        errors:     错误信息列表
        unique_id:  当前节点编号
    """
    unique_id = item
    ...
    # ── 第一步：获取节点的输入类型定义和自定义验证函数 ────────────────────────
    v3_data = None
    validate_function_inputs = []   # 自定义 VALIDATE_INPUTS 函数接收的参数名列表
    validate_has_kwargs = False     # 自定义验证函数是否接受 **kwargs
    if issubclass(obj_class, _ComfyNodeInternal):
        # V3 新式节点：使用 validate_inputs 方法
        ...
        # 旧式节点：使用 VALIDATE_INPUTS 静态方法（图片走的是这里）
        class_inputs = obj_class.INPUT_TYPES()#此处返回是字符串（required对象）
        validate_function_name = "VALIDATE_INPUTS"
        validate_function = getattr(obj_class, validate_function_name, None)#动态抓取方法，该处方法为（VALIDATE_INPUTS）

    if validate_function is not None:
        # 获取自定义验证函数的参数列表，用于后续按需传参
        argspec = inspect.getfullargspec(validate_function)
        validate_function_inputs = argspec.args
        validate_has_kwargs = argspec.varkw is not None

    received_types = {}  # 记录每个连接输入实际接收到的类型，供自定义验证函数使用

    # required + optional 的所有合法输入名集合
    ...
    for x in valid_inputs:
        ...
        if isinstance(val, list):
            # ── 分支A：该输入是节点连接（来自另一个节点的输出）──────────────
            # 连接格式必须是 [来源节点ID, 输出槽索引]，长度必须为 2
            if len(val) != 2:
                error = {
            ...
            received_type = r[val[1]]   # 来源节点对应输出槽的类型
            received_types[x] = received_type

            # 校验来源节点输出类型和当前节点期望类型是否匹配
            ...
            # 递归验证来源节点本身是否合法
            try:
                #检验输入合法validate_inputs
                r = await validate_inputs(prompt_id, prompt, o_id, validated)
                if r[0] is False:
                    # 来源节点验证失败，结果已缓存到 validated[o_id]，标记当前节点也无效
                    valid = False
                    continue
            except Exception as ex:
                typ, _, tb = sys.exc_info()
            ...
            # ── 分支B：该输入是直接填写的值（非连接）──────────────────────
            try:
                # 如果值是用 __value__ 包裹的字典（列表组件的特殊格式），先解包
                if isinstance(val, dict):
                    if "__value__" in val:
                        val = val["__value__"]
                        inputs[x] = val

                # 按期望类型做强制类型转换，确保数据类型正确
                if input_type == "INT":
                    val = int(val)
                    inputs[x] = val
                if input_type == "FLOAT":
    ...
    if len(validate_function_inputs) > 0 or validate_has_kwargs:
        ...
        # 如果自定义验证函数声明了 input_types 参数，传入各连接输入的实际类型
        ...
        for x in input_filtered:
            for i, r in enumerate(ret):
                # 返回 True 或 ExecutionBlocker 表示通过，其余视为失败
                if r is not True and not isinstance(r, ExecutionBlocker):
                    details = f"{x}"
                    if r is not False:
                        details += f" - {str(r)}"  # 非 False 的返回值作为额外错误说明
...
async def validate_prompt(prompt_id, prompt, partial_execution_list: Union[list[str], None]):
    """
    验证整个 prompt（工作流）是否合法，可以被执行。

    参数：
        prompt_id: 本次任务的唯一 ID
        prompt: 完整的工作流字典，key 是节点编号，value 是节点结构
        partial_execution_list: 局部执行的节点 ID 列表；None 表示执行所有输出节点

    返回：四元组 (is_valid, error, good_outputs, node_errors)
        is_valid:      bool，整个 prompt 是否通过验证
        error:         全局错误信息字典（通过时为 None）
        good_outputs:  验证通过的输出节点 ID 列表
        node_errors:   各节点的错误信息字典
    """
    # ── 第一步：扫描所有节点，找出输出节点 ──────────────────────────────────
    outputs = set()
    for x in prompt:
        # 检查节点是否有 class_type 字段（基本结构校验）
        ...
        # 在全局节点注册表中查找该 class_type 对应的节点类
        ...
        # 如果该节点是输出节点（如 SaveImage），加入 outputs 集合
        # partial_execution_list 为 None 表示全量执行，否则只执行指定节点
    ...
    # ── 第三步：对每个输出节点递归验证其上游输入 ─────────────────────────────
    good_outputs = set()   # 验证通过的输出节点
    errors = []            # 验证失败的输出节点及原因
    node_errors = {}       # 各节点的具体错误信息（用于前端高亮报错节点）
    validated = {}         # 缓存已验证过的节点结果，避免重复验证
    ...
    for o in outputs:
        ...
        if valid is True:
            ...
            # 遍历所有已验证过的节点，找出有错误的节点记录到 node_errors
            for node_id, result in validated.items():
                valid = result[0]
                reasons = result[1]
                # 上游节点出错时，下游节点也会被标记为无效，但 reasons 为空
                # 只记录真正有错误信息的节点，避免干扰前端报错定位
