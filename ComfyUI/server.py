        ...
        if 'Host' in request.headers and 'Origin' in request.headers:
            ...
            if loopback and host_domain is not None and origin_domain is not None and len(host_domain) > 0 and len(origin_domain) > 0:
                if host_domain != origin_domain:
                    ...
                    logging.warning("WARNING:没什么吊用的 403".format(host_domain, origin_domain))
                    # return web.Response(status=403)  # 注释掉以允许本地跨域开发(Rylynn)
...
class PromptServer():
    def __init__(self, loop):
        ...
        async def post_prompt(request):
            """
            处理 POST /prompt 请求 —— 这是提交工作流执行的核心接口
            前端把整个工作流(节点图)以JSON格式发过来，这个方法负责验证并放入执行队列
            """
            ...
            # 触发所有注册的 on_prompt 钩子函数（插件可以在这里拦截/修改请求）
            json_data = self.trigger_on_prompt(json_data)

            # 2. 确定任务的优先级编号（number越小优先级越高）
            if "number" in json_data:
                ...
                if "front" in json_data:
                    if json_data['front']:
                        # front=true 表示插队，用负数让它排到队列最前面
                        number = -number

                self.number += 1

            # 3. 核心逻辑：处理 prompt（工作流节点图）
            if "prompt" in json_data:
                # prompt 就是那个包含所有节点定义和连接关系的字典
                # 例如: {"3": {"class_type": "KSampler", "inputs": {...}}, "4": {...}}
                prompt = json_data["prompt"]

                # 生成唯一的任务ID，客户端也可以自己指定
                prompt_id = str(json_data.get("prompt_id", uuid.uuid4()))

                # 部分执行目标：允许只执行工作流中的部分节点（可选功能）
                partial_execution_targets = None
                if "partial_execution_targets" in json_data:
                    partial_execution_targets = json_data["partial_execution_targets"]

                # 应用节点替换规则（比如把旧版节点自动替换成新版）
                self.node_replace_manager.apply_replacements(prompt)

                # 4. 验证工作流是否合法
                # 检查所有节点的 class_type 是否存在、输入类型是否匹配、连接是否正确
                valid = await execution.validate_prompt(prompt_id, prompt, partial_execution_targets)

                # 提取附加数据（如前端工作流布局信息 extra_pnginfo，会嵌入到输出的PNG元数据中）
                extra_data = {}
                if "extra_data" in json_data:
                    extra_data = json_data["extra_data"]

                # client_id 用于WebSocket通信，确保执行结果推送给正确的客户端
                if "client_id" in json_data:
                    extra_data["client_id"] = json_data["client_id"]

                if valid[0]:
                    # 5. 验证通过，放入执行队列
                    outputs_to_execute = valid[2]  # 需要执行的输出节点（如 SaveImage）

                    # 分离敏感数据（如API密钥），不会出现在历史记录中
                    sensitive = {}
                    for sensitive_val in execution.SENSITIVE_EXTRA_DATA_KEYS:
                        if sensitive_val in extra_data:
                            sensitive[sensitive_val] = extra_data.pop(sensitive_val)

                    extra_data["create_time"] = int(time.time() * 1000)  # 记录创建时间戳（毫秒）

                    # 放入优先级队列，后台的 prompt_worker 线程会取出并执行
                    # 元组结构: (优先级, 任务ID, 工作流, 附加数据, 输出节点列表, 敏感数据)
                    self.prompt_queue.put((number, prompt_id, prompt, extra_data, outputs_to_execute, sensitive))

                    # 返回任务ID给客户端，客户端可以用这个ID查询执行状态和结果
                    response = {"prompt_id": prompt_id, "number": number, "node_errors": valid[3]}
                    return web.json_response(response)
                else:
                    # 6. 验证失败，返回400错误和具体的错误信息
                    logging.warning("invalid prompt: {}".format(valid[1]))
                    return web.json_response({"error": valid[1], "node_errors": valid[3]}, status=400)
            else:
                # 7. 请求中没有 prompt 字段，返回错误
                error = {
                    "type": "no_prompt",
