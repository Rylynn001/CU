  ...
  if (activeTab.value === 'img2img') {
    ...
    for (const asset of assets) {
      ...
      inputImages.value.push({
        file: null,
        preview: `/api/view?filename=${encodeURIComponent(asset.location.split('/').pop()!)}&type=output`,
    ...
    if (img2img) {
      ...
      for (const img of snapshotImages) {
        if (img.file) {
          ...
          const imgRes = await fetch(`/api/view?filename=${encodeURIComponent(img.assetLocation.split('/').pop()!)}&type=output`)
